...
void Game::Update()
{
	...
	glm::vec2 moveinput = Input::GetMoveInput();
	Renderer::camera.transform = Renderer::camera.transform * glm::translate(glm::mat4(1.0f), (glm::vec3(0, 0, 1) * -moveinput.y + glm::vec3(1, 0, 0) * moveinput.x) * moveSpeed * (float)Renderer::GetDeltaTime());
